...
export default function ShopperForm({ onShopperUpdated, editData }) {
    ...
    const handleFileUpload = (newFileKey) => {
        ...
        setFormData((prev) => ({ ...prev, imageUrl: `https://${process.env.S3_BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/${newFileKey}` }));
    };
        ...
        <Box sx={{ px: 2 }}>
            ...
            <form onSubmit={handleSubmit}>
                <Stack spacing={3}>
                    ...
                    {activeStep === 0 && (
                        <>
                            <TextField label="Shopper Name" name="shopperName" value={formData.shopperName} onChange={handleChange} fullWidth required />
                            ...
                            <TextField label="Cashier Name" name="cashier" value={formData.cashier} onChange={handleChange} fullWidth required />
                        </>
                    )}
                    ...
                    {activeStep === 1 && (
                        <>
                            <TextField label="Food Score" name="foodScore" type="number" value={formData.foodScore} onChange={handleChange} fullWidth required />
                            <TextField label="Service Score" name="serviceScore" type="number" value={formData.serviceScore} onChange={handleChange} fullWidth required />
                            <TextField label="Cleanliness Score" name="cleanScore" type="number" value={formData.cleanScore} onChange={handleChange} fullWidth required />
                            <TextField label="Final Score (Auto-Calculated)" name="finalScore" type="number" value={formData.finalScore} fullWidth disabled />
                        </>
                    )}
                    ...
                    {activeStep === 2 && (
                        <>
                            <TextField label="Comments" name="comments" multiline rows={3} value={formData.comments} onChange={handleChange} fullWidth />
